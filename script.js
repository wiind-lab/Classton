...
function openSettingsWithPassword() {
  ...
  if (input === pw) {
    openSettingsDrawer(); /* [v7-2] drawer UI로 전환 */
  } else {
...
function updateGroupViewButton() {
  /* [v7-B.3] groupViewSwitch DOM 제거됨 — 상태 칩만 업데이트 */
  _updateStatusBadges();
}
...
function updateRoleViewButton() {
  /* [v7-B.3] roleViewSwitch DOM 제거됨 — 상태 칩만 업데이트 */
  _updateStatusBadges();
}
...
function _updateStatusBadges() {
  /* [v7-B.2] 상태 칩: ON 상태만 표시, 2개(모둠보기/1인1역)만 사용 */
  const area = document.getElementById('statusBadgeArea');
  if (!area) return;
  area.innerHTML = '';
  const _chip = (label, isOn) => {
    if (!isOn) return;                           /* OFF 상태는 표시하지 않음 */
    const b = document.createElement('span');
    b.className = 'status-chip status-chip-on';
    b.innerHTML = `<span class="status-chip-dot"></span>${label}`;
    area.appendChild(b);
  };
  _chip('모둠보기', groupViewOn);
  _chip('1인1역',   roleViewOn);
  /* 성별같이앉기: genderActiveBadge(버튼 옆 점)로만 표시 — 칩 제거 */
}
...
/* [v7-2] ESC 키로 Settings Drawer 닫기 */
